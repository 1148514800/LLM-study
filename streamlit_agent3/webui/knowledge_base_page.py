...
from utils import PLATFORMS, get_embedding_models, get_kb_names, save_kb_config, load_kb_config
...
def knowledge_base_page():
    ...
    if selected_kb == "新建知识库":
        ...
        with status_placeholder.status("知识库配置", expanded=True) as s:
            ...
            embedding_models = get_embedding_models(platform)   # 这里返回的只是一个列表
            ...
            if submit and kb_name.strip():
                ...
                # 存放知识库信息到config/kb_config.json中
        ...
        if upload:
            ...
            kb_config = load_kb_config(kb_name = selected_kb)

            embedding_model = kb_config.get("embedding_model", "")
            # platform = kb_config.get("platform", "")
            ...
            vectorstore  = Chroma.from_documents(
                ...
                # embedding=get_embedding_model(platform_type=""),
                embedding=get_embedding_model(platform_type=embedding_model),
                persist_directory=vs_path  # 允许我们将persist_directory目录保存到磁盘上
            )
            vectorstore.persist()
            # 显示上传成功的信息
            st.success("上传文件成功")
