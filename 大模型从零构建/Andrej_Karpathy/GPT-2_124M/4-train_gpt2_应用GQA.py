...
class MLP(nn.Module):
    def __init__(self, config):
        ...
        self.linear_gate = nn.Linear(config.n_embd, config.n_embd) 
        # 门控结构主干线性层 
        self.linear_up = nn.Linear(config.n_embd, config.n_embd)
        # 定义一个线性层，输入输出都是门控结构的尺寸
        self.linear_down = nn.Linear(config.n_embd, config.n_embd)
        # 初始化一个随机数作为beta系数  
