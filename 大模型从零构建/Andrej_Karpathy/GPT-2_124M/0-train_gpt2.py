...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # 总结来说，FlashAttention 是 F.scaled_dot_product_attention() 的一种高效实现选项，用户可以通过配置选择使用它，或者让 PyTorch 自动根据输入和硬件条件选择是否使用
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        # forward the token and posisition embeddings：这里为绝对位置编码
...
t1 = time.time()
print(f'没加上cache的生成速度为:{(t1-t0) * 1000:.4f}ms')
